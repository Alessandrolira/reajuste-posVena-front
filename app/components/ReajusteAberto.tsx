...
import { InteracaoType } from "../types/TypeTabela";
...
export default function ReajusteAberto({
  ...
  function ahNegociacaoAceita(interacoes: InteracaoType[]) {
    for (let i = 0; i < interacoes.length; i++) {
      if (interacoes[i].isAceita == true) {
        return true;
      }
    }
    return false;
      ...
      <div className="basis-1/3 flex items-start justify-between ">
        {interacoes.length == 0 ? (
          <div className="flex items-center justify-between w-full">
            ...
            <button
              onClick={onClickAbrirNegociacao}
              className="bg-(--azul-escuro) text-(--branco) px-3 py-1 rounded-lg hover:bg-[#9ea9b6] cursor-pointer  transition-colors duration-100 text-right"
            >
              Abrir Negociação
            </button>
          </div>
